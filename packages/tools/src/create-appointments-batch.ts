...
async function resolveStaffIdByName(
    ...
    if (!response.ok) {
        return {
            error: "Personel doÄŸrulamasÄ± yapÄ±lamadÄ±.",
    ...
    if (matches.length === 0) {
        return {
            error: `"${staffName}" isimli bir personel bulunamadÄ±.`,
    ...
    if (matches.length > 1) {
        return {
            error: `"${staffName}" birden fazla personel ile eÅŸleÅŸti. LÃ¼tfen daha net bir isim verin.`,
...
export async function createAppointmentsBatch(
    ...
    if (serviceNames.length === 0) {
        return {
            ...
            error: "'service_names' en az bir hizmet iÃ§ermelidir.",
        };
    }
    ...
    if (!isIsoDate(date)) {
        return {
            success: false,
            code: "validation_error",
            error: "'date' YYYY-MM-DD formatÄ±nda olmalÄ±dÄ±r.",
        };
    }
    ...
    if (!isHourMinute(startTime)) {
        return {
            success: false,
            code: "validation_error",
            error: "'start_time' HH:MM formatÄ±nda olmalÄ±dÄ±r.",
        };
    }
    ...
    if (!staffId && staffNameFromArgs) {
        const resolved = await resolveStaffIdByName(
            config,
            ctx.tenantId,
            staffNameFromArgs
        );
        if (!resolved.staffId) {
            return {
                ...
                error: resolved.error || "Personel doÄŸrulanamadÄ±.",
            };
        }
    ...
    if (!staffId) {
        return {
            success: false,
            code: "validation_error",
            error: "Ã‡oklu randevu iÃ§in 'staff_id' veya 'staff_name' zorunludur.",
        };
    }
    ...
    const response = await fetch(
        `${config.url}/rest/v1/rpc/create_appointments_batch_atomic`,
        {
            method: "POST",
            headers: {
    ...
    if (!response.ok) {
        ...
        return {
            success: false,
            code: "rpc_error",
            error: "Ã‡oklu randevu oluÅŸturulamadÄ±.",
    ...
    return {
        ...
        error: "Ã‡oklu randevu RPC geÃ§ersiz yanÄ±t dÃ¶ndÃ¼rdÃ¼.",
    };
}
