...
export async function composeInteractiveMessage(
  ...
  if (!body) {
    return {
      success: false,
      error: "ArgÃ¼man hatasÄ±: 'body' zorunludur",
    };
  }
  ...
  if (kind === "buttons") {
    ...
    if (buttons.length === 0) {
      return {
        success: false,
        error: "ArgÃ¼man hatasÄ±: 'buttons' en az 1 geÃ§erli Ã¶ÄŸe iÃ§ermeli",
      };
    }
  ...
  if (kind === "list") {
    ...
    if (!buttonText) {
      return {
        success: false,
        error: "ArgÃ¼man hatasÄ±: 'button_text' zorunludur (list iÃ§in)",
      };
    }
    ...
    if (sections.length === 0) {
      return {
        success: false,
        error: "ArgÃ¼man hatasÄ±: 'sections' en az 1 geÃ§erli bÃ¶lÃ¼m iÃ§ermeli",
      };
    }
  ...
  return {
    success: false,
    error: "ArgÃ¼man hatasÄ±: 'kind' yalnÄ±zca 'buttons' veya 'list' olabilir",
  };
}
