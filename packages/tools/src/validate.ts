    ...
    for (const field of fields) {
        ...
        if (field.type === "string") {
            ...
            if (field.pattern && !field.pattern.test(String(value))) {
                errors.push(
                    `'${field.name}' geÃ§ersiz format${field.patternHint ? ` (${field.patternHint})` : ""}`
                );
            }
        } else if (field.type === "number") {
            const num = Number(value);
            if (isNaN(num)) {
                errors.push(`'${field.name}' geÃ§erli bir sayÄ± olmalÄ±dÄ±r`);
            } else {
                data[field.name] = num;
            }
            ...
            if (typeof value === "string") {
                ...
                errors.push(`'${field.name}' boolean olmalÄ±dÄ±r`);
            }
        }
    }

    if (errors.length > 0) {
        return { valid: false, error: `ArgÃ¼man hatasÄ±: ${errors.join(", ")}` };
    }
...
export const CREATE_APPOINTMENT_FIELDS: FieldSpec[] = [
    ...
    { name: "staff_id", type: "string", required: true },
    {
        ...
        patternHint: "ISO 8601: YYYY-MM-DDTHH:MM:SS+03:00 (Ã¶rnek: 2026-03-05T14:00:00+03:00)",
    },
